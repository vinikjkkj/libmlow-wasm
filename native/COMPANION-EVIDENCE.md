...
Worth recording how it was nearly missed: an earlier reading of the binary
gave these eleven slots as "5 correlations, 2 parameters, 3 log10, 1 int16",
which is what the trace now confirms. It was replaced with the reference
layout on the strength of the widths matching. Matching widths were never
evidence about order or transform.
...
established that the ratio should be near 1.** That was an assumption about
...
Eight numerical coincidences in this work have looked like answers and been
...
does not use. That is the eighth time in this work a change improved a metric
...
Measured 2026-09-21, after a reading of the client binary predicted it. The
...
A trained network does not run with a third of its `tanh` outputs at ±1. A
reading of the client puts its filter gains within about 30%, which is
...
## A second routing, measured both ways

A second look at the binary reads the client's caller as feeding `ft1` and `ft2` the *same*
...
second reading's own mechanism predicts: with the GRU already saturated, routing a
...
| `ft2` reading the GRU (second reading) | **1.3%** |
...
- **`alpha1_f` is int8 and blocked; `alpha1_t` and `alpha2` are float and
  ...
  cannot cross. A reading that they had been crossed is ruled out here.
...
A correction to a proposal made on the back of the 4.7 dB: that the
...
**Something nobody has thought to check.** The honest second entry. The
...
not the data. Fixed in `native/companion_test.c`; the measurement harness is not
...
The shim and the loading scripts are not part of this repository.
...
Worth its own warning, because it silently rewrote the reference dump and
...
mid-investigation, and the baseline silently moved from +2.90 to +4.73. The tell was
...
a harness outside this repository and written to a text
